...
public:
    ...
    {
        char* dest;
        dest = new char[MAX_ID_LENGTH + MAX_NICKNAME_LENGTH + sizeof(flag_) +
            sizeof(content_length_) + (content_length_ + 1) ];
        std::size_t pt = 0;
    ...
    const std::size_t content_length() { return content_length_; }
    const std::size_t content_length() const { return content_length_; }
...
public:
    ...
    chat_message_header(const std::size_t _body_size, const std::size_t _message_type)
        : body_size(_body_size), message_type(_message_type)
    {
    ...
    bool is_valid()
    {
        ...
        for (std::size_t mt = MT_FRONTVALUE + 1; mt < MT_LASTVALUE; mt++)
        {
...
public:
    cm_room_message(const char* _id, const char* _room_name, long long _room_number = 0,
        std::size_t _participants_num = 0)
        : room_number(_room_number), participants_num(_participants_num),
    ...
    char* serialization()
    {
        ...
        std::size_t length = sizeof(id) + sizeof(room_number) +
            ...
            (MAX_ID_LENGTH * participants_num);
        ser_buf = new char[length];
        std::size_t pt = 0;
        ...
        pt += sizeof(participants_num);
        for (std::size_t i = 0; i < participants_num; i++)
        {
    ...
    void set_room_participants_id(std::set<participant_ptr>& participants_set)
    {
        if(room_participants != nullptr) return;
        participants_num = participants_set.size();
        room_participants = new char*[participants_num];

        auto it = participants_set.begin();
        for(std::size_t i = 0; i < participants_num; i++)
        {
            auto id_length = (*it)->id().size();
            room_participants[i] = new char[MAX_ID_LENGTH];
            std::memcpy(room_participants[i], (*it)->id().c_str(), id_length + 1);
            it++;
        }
    }

    void set_room_participants_id(ThreadedSet<participant_ptr>& participants_set)
    {
        if(room_participants != nullptr) return;
        participants_num = participants_set.size();
        room_participants = new char*[participants_num];
    ...
    {
        std::size_t pt = MAX_ID_LENGTH + sizeof(room_number) +
    ...
    std::size_t size()
    {
        std::size_t ret = sizeof(id) + sizeof(room_number) +
            sizeof(room_name) + sizeof(participants_num) +
