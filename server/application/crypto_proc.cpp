...
crypto_proc::crypto_proc(bool init)
{
	if(init)
	{
...
void crypto_proc::ToHash(const std::string& plain, std::string& output)
{
	CryptoPP::SHA3_256 sha256;
    CryptoPP::StringSource(plain, true, new CryptoPP::HashFilter(sha256,
...
void crypto_proc::EncryptWithRSA(const unsigned char* plain, unsigned char* output, std::size_t& output_len)
{
	...
	for (size_t i = 0; i < plainDataLen; i += fixedLen)
	{
		...
		CryptoPP::PK_EncryptorFilter *ef
